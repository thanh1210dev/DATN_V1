...
const ColorAdmin = () => {
  ...
  const [formData, setFormData] = useState({ code: '#000000', name: '' });
  ...
  const fetchColors = async () => {
    try {
      const data = await ColorService.getAll(page, size);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await ColorService.update(editingId, payload);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Quản lý màu sắc</h2>
        <button
          onClick={handleAdd}
        ...
        <table className="w-full text-sm text-left text-gray-700">
          <thead className="text-xs font-semibold uppercase bg-indigo-50 text-indigo-700">
            <tr>
              ...
              <th className="px-6 py-3 w-32">Màu</th>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 w-full max-w-md shadow-xl">
            <h3 className="text-xl font-semibold mb-6 text-gray-800">
              {isEditing ? 'Cập nhật màu sắc' : 'Thêm màu sắc mới'}
            </h3>
            <form onSubmit={handleSubmit}>
              <div className="mb-5">
                <label className="block text-sm font-medium text-gray-700 mb-1">Mã màu sắc (Hex)</label>
                <input
                  type="text"
                  name="code"
                  value={formData.code}
