...
const ProductAdmin = () => {
  ...
  const fetchComboboxData = async () => {
    try {
      const brandData = await BrandService.getAll(0, 100); // Fetch all brands
      setBrands(brandData.content.map(item => ({ value: item.id, label: item.name })));

      const materialData = await MaterialService.getAll(0, 100); // Fetch all materials
      setMaterials(materialData.content.map(item => ({ value: item.id, label: item.name })));

      const categoryData = await CategoryService.getAll(0, 100); // Fetch all categories
